// frontend/src/pages/GestaoDadosApoioPage.jsx
...
// --- Componente Genérico para o Modal de Edição/Criação ---
// Ele pode ser usado tanto para OBMs quanto para Naturezas
function DataModal({ item, type, onClose, onSave }) {
  const isEditing = !!item;
  const isObm = type === 'obm';
  
  // O formulário se adapta ao tipo de dado
...
function GestaoDadosApoioPage() {
  const [activeTab, setActiveTab] = useState('obms'); // 'obms' ou 'naturezas'
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setError(err.message || 'Falha ao buscar dados de apoio.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleSave = async (formData) => {
    try {
      if (activeTab === 'obms') {
        itemEmEdicao ? await updateObm(itemEmEdicao.id, formData) : await createObm(formData);
      } else {
        itemEmEdicao ? await updateNatureza(itemEmEdicao.id, formData) : await createNatureza(formData);
      }
      alert('Dados salvos com sucesso!');
      handleCloseModal();
      fetchData();
    } catch (err) {
      alert(`Erro: ${err.response?.data?.message || err.message || 'Falha ao salvar dados.'}`);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Tem certeza que deseja excluir este item? Esta ação não pode ser desfeita.')) {
      try {
        ...
        alert(`Erro: ${err.response?.data?.message || err.message || 'Falha ao excluir item.'}`);
      }
    }
  };
      ...
      <>
        ...
        <table style={styles.table}>
          ...
          <tbody>
            {data.map(item => (
              <tr key={item.id}>
                {columns.map(col => <td key={col.key} style={styles.td}>{item[col.key]}</td>)}
                <td style={styles.td}>
