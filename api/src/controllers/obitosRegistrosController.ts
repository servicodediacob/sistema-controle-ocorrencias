...
export const getObitosPorData = async (req: RequestWithUser, res: Response) => {
  ...
  try {
    ...
    logger.error({ err: error, data }, 'Erro ao buscar registros de 贸bito.');
...
export const criarObitoRegistro = async (req: RequestWithUser, res: Response) => {
  ...
  try {
    ...
    logger.info({ registro: novoRegistro, usuarioId: usuario_id }, 'Novo registro de 贸bito criado.');
    return res.status(201).json(novoRegistro);
  } catch (error) {
    logger.error({ err: error, body: req.body }, 'Erro ao criar registro de 贸bito.');
    return res.status(500).json({ message: 'Erro interno do servidor.' });
  }
};
...
export const atualizarObitoRegistro = async (req: RequestWithUser, res: Response) => {
    ...
    try {
        ...
        logger.info({ registroId: id, usuarioId: req.usuario?.id }, 'Registro de 贸bito atualizado.');
        return res.status(200).json(registroAtualizado);
    } catch (error) {
        logger.error({ err: error, params: req.params, body: req.body }, 'Erro ao atualizar registro de 贸bito.');
        return res.status(500).json({ message: 'Erro interno do servidor.' });
    }
};
