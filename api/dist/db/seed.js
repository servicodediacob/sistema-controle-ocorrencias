...
const index_1 = __importDefault(require("./index")); // Importa a instância do pool de conexão
require("../config/envLoader"); // Garante que as variáveis de ambiente sejam carregadas
...
async function seedDevelopmentDatabase() {
    ...
    const client = await index_1.default.pool.connect();
    ...
    `);
        ...
        await client.query(`INSERT INTO usuarios (nome, email, senha_hash, role) VALUES ($1, $2, $3, 'admin') ON CONFLICT (email) DO NOTHING`, ['Admin Padrão', 'admin@example.com', adminSenhaHash]);
